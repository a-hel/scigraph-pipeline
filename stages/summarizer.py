...
class Summarizer:
    def __init__(self,
                 ...
                 checkpoint_dir=os.path.join(os.getenv('ASSET_DIR'), os.getenv('SCITLDR_MODELDIR')),
    ...
    def summarize(self, article, only_conclusion=False):
        ...
        if only_conclusion:
            ...
            article = [
                " ".join([article[section] for section in ['Abstract', 'Introduction', 'Conclusion']]),]
        article = [art.replace('\n', ' ') for art in article]
        with torch.no_grad():
...
def summarize_article(articles, checkpoint_file=None):
    checkpoint_file = checkpoint_file or os.getenv('CHECKPOINT_FILE').replace(os.path.sep, '.')
    summarizer = Summarizer(checkpoint_file=checkpoint_file,
        max_len_b=50)
    for article in articles:
        try:
            ...
            summary = None
            conclusion = None
            error = ("Summarization error:\t%s\t%s\t%s\n" % (article['doi'], article["origin"], error))
