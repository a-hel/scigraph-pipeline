...
class GraphWriter:
    ...
    def _serialize_props(self, data):
        if not data:
            ...
            labels = [
                f"{label}:{self._format_value(value)}" for label, value in data.items() if value is not None
            ]
    ...
    def edge_create_stmt(self, edgetype, nodetype_left, nodetype_right,
    join_key_left, join_key_right, join_value_left, join_value_right, _props):
    ...
    encoding='utf-8', delete=False, newline=os.linesep, dir=temp_dir) as f:
            temp_file = os.path.join(temp_dir, f.name)
            
            writer = csv.writer(f)
            writer.writerow(columns)
            for record in batch:
                data = {"conclusion": record.summary_id.conclusion,
                ...
                writer.writerow(row)
                print(",".join(row))
        query = f"LOAD CSV WITH HEADERS FROM {temp_file}" + \
            stmt_func
        if write:
    ...
    def batch_load_nodes(self, node_type, nodes, batch_size=10_000, write=True):
        ...
        try:
            ...
            raise KeyError(f"Invalid node type: '{node_type}. Allowed are '{', '.join(self.node_props.keys())}'")
    ...
    def batch_load_edges(self, edge_type, edges, batch_size=10_000, write=True):
        fields = ["edgetype", "nodetype_left", "nodetype_right",
    "join_key_left", "join_key_right", "join_value_left", "join_value_right"]
        try:
            props = self.edge_props[edge_type]
        except KeyError:
            raise KeyError(f"Invalid edge type: '{edge_type}. Allowed are '{', '.join(self.edge_props.keys())}'")
        fields_data = {col: f"row.{col}" for col in fields}
        props_data = {col: f"row.{col}" for col in props}
        data = fields_data
        data["_props"] = props_data
